...
void main() {
  ...
  group('recoverPublicKeyFromSignature', () {
    test('should recover a public key', () {
      ...
      var v = 27;
      var pubkey = signature.recoverPublicKeyFromSignature(signature.ECDSASignature(r, s, v), echash);
    ...
    test('should recover a public key (ropstenChainId = 3)', () {
      ...
      var v = 41;
      var pubkey =
          signature.recoverPublicKeyFromSignature(signature.ECDSASignature(r, s, v), echash, chainId: ropstenChainId);
      expect(pubkey, signature.privateKeyToPublicKey(ecprivkey));
    });
    ...
    test('should fail on an invalid signature (v = 21)', () {
      ...
      var s = decodeBigInt(hex.decode('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66'));
      var v = 21;
      expect(() => signature.recoverPublicKeyFromSignature(signature.ECDSASignature(r, s, v), echash),
          throwsArgumentError);
    });
    ...
    test('should fail on an invalid signature (swapped points)', () {
      ...
      var s = decodeBigInt(hex.decode('129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66'));
      var v = 27;
      expect(() => signature.recoverPublicKeyFromSignature(signature.ECDSASignature(s, r, v), echash),
          throwsArgumentError);
    });
  });
  ...
  group('isValidSignature', () {
    ...
    test('should fail when on homestead and s > secp256k1n/2', () {
      var SECP256K1_N_DIV_2 =
          decodeBigInt(hex.decode('7fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681b20a0'));
  ...
  group('message sig', () {
    ...
    test('pad short r and s values', () {
      const v = 27;
      expect(
          signature.toRpcSig(decodeBigInt(Uint8List.view(encodeBigInt(r).buffer, 20)),
              decodeBigInt(Uint8List.view(encodeBigInt(s).buffer, 20)), v),
