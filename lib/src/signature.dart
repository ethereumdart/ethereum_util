...
ECDSASignature sign(Uint8List message, Uint8List privateKey, {int chainId = -1}) {
  ...
  /*
	This is necessary because if a message can be signed by (r, s), it can also	be signed by (r, -s (mod N)) which N being the order of
  the elliptic function	used. In order to ensure transactions can't be tampered with (even though it	would be harmless), Ethereum only
  accepts the signature with the lower value	of s to make the signature for the message unique.
	More details at	https://github.com/web3j/web3j/blob/master/crypto/src/main/java/org/web3j/crypto/ECDSASignature.java#L27
	 */
  ...
  if (recoveryId == -1) throw Exception('Could not construct a recoverable key. This should never happen');
...
bool isValidSignature(BigInt r, BigInt s, int v, {bool homesteadOrLater = true, int chainId = -1}) {
  ...
  if (encodeBigInt(r).length != 32 || encodeBigInt(s).length != 32) return false;
  if (!_isValidSigRecovery(_calculateSigRecovery(v, chainId: chainId))) return false;
  if (r == BigInt.zero || r > SECP256K1_N || s == BigInt.zero || s > SECP256K1_N) return false;
  if (homesteadOrLater && s > SECP256K1_N_DIV_2) return false;

  return true;
}

Uint8List recoverPublicKeyFromSignature(ECDSASignature sig, Uint8List message, {int chainId = -1}) {
  int recoveryId = _calculateSigRecovery(sig.v, chainId: chainId);

  if (!_isValidSigRecovery(recoveryId)) throw ArgumentError('invalid signature v value');
  if (!isValidSignature(sig.r, sig.s, sig.v, chainId: chainId)) throw ArgumentError('invalid signature');
...
Uint8List _recoverPublicKeyFromSignature(int recId, BigInt r, BigInt s, Uint8List message) {
  ...
  if (x.compareTo(prime) >= 0) return Uint8List.fromList([]);

  final R = _decompressKey(x, (recId & 1) == 1, params.curve);
  if (!(R! * n)!.isInfinity) return Uint8List.fromList([]);
...
ECDSASignature fromRpcSig(String sig) {
  ...
  if (buf.length != 65) throw ArgumentError('Invalid signature length');
