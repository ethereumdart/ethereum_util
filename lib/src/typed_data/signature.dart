...
class SignatureUtil {
  ...
  static String sign({required Uint8List message, required Uint8List privateKey}) {
    final sig = signToSignature(message, privateKey);
    return concatSig(toBuffer(sig.r), toBuffer(sig.s), toBuffer(sig.v));
  }

  static String signPersonalMessage({required Uint8List message, required Uint8List privateKey}) {
  ...
  static String ecRecover(
    ...
    final publicKey = recoverPublicKeyFromSignature(SignatureUtil.fromRpcSig(signature), messageHash);
    if (publicKey == null) throw Exception('Can not recover public key from signature');
  ...
  static String concatSigCompact(Uint8List r, Uint8List s) {
    var rSig = fromSigned(r);
    var sSig = fromSigned(s);
    var rStr = _padWithZeroes(hex.encode(toUnsigned(rSig)), 64);
    var sStr = _padWithZeroes(hex.encode(toUnsigned(sSig)), 64);
    return addHexPrefix(rStr + sStr);
  }
