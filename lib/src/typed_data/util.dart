...
class TypedDataUtil {
  static Uint8List hashMessage({required String jsonData, required TypedDataVersion version, int? chainId}) {
    ...
    if (version == TypedDataVersion.V1) {
      ...
      try {
        if (rawTypedData is List) {
          typedData = rawTypedData.map((e) => EIP712TypedData.fromJson(e)).toList();
        } else {
          typedData = [EIP712TypedData.fromJson(rawTypedData)];
        }
      } catch (_) {
        throw ArgumentError('jsonData format is not corresponding to EIP712TypedData');
      }
    ...
    try {
      ...
      throw ArgumentError('jsonData format is not corresponding to TypedMessage');
    }

    return version == TypedDataVersion.V4 ? TypedDataUtil.hashTypedDataV4(typedData): TypedDataUtil.hashTypedDataV3(typedData);
  }
  ...
  static Uint8List? recoverPublicKey(dynamic data, String sig, TypedDataVersion version, {int? chainId}) {
    var sigParams = fromRpcSig(sig);
    var messageHash;
    switch (version) {
      case TypedDataVersion.V1:
        if (!(data is List<EIP712TypedData>)) throw ArgumentError('Recover public key version 1 required EIP712TypedData object');

        messageHash = hashTypedDataV1(data);
        break;

      case TypedDataVersion.V3:
        if (!(data is TypedMessage)) throw ArgumentError('Recover public key version 3 required TypedMessage object');

        messageHash = hashTypedDataV3(data);
        break;

      case TypedDataVersion.V4:
        if (!(data is TypedMessage)) throw ArgumentError('Recover public key version 4 required TypedMessage object');
  ...
  static Uint8List hashTypedData(TypedMessage typedData, String version) {
    ...
    parts.add(hashStruct('EIP712Domain', typedData.domain, typedData.types, version));

    if (typedData.primaryType != 'EIP712Domain') {
      parts.add(hashStruct(typedData.primaryType, typedData.message, typedData.types, version));
    }
    return keccak256(parts.toBytes());
  }

  static Uint8List hashStruct(String primaryType, dynamic data, Map<String, List<TypedDataField>> types, String version) {
  ...
  static Uint8List hashType(String primaryType, dynamic types) {
    return keccak256(Uint8List.fromList(utf8.encode(encodeType(primaryType, types))));
  }

  static Uint8List encodeData(String primaryType, dynamic data, Map<String, List<TypedDataField>> types, String version) {
    if (!(data is Map<String, dynamic>) && !(data is EIP712Domain)) throw ArgumentError("Unsupported data type");
    ...
    if (version == 'V4') {
      List<dynamic> encodeField(String name, String type, dynamic value) {
        ...
        if (value == null) throw ArgumentError('missing value for field ${name} of type ${type}');
        ...
        if (type.lastIndexOf(']') == type.length - 1) {
          ...
          final typeValuePairs = value.map((item) => encodeField(name, parsedType, item)).toList();

          final List<String> tList = (typeValuePairs as List).map((l) => l[0].toString()).toList();
