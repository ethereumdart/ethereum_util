...
class AbiUtil {
  ...
  static Uint8List solidityHexValue(String type, value, bitsize) {
    ...
    if (isArray(type)) {
      ...
      return Uint8List.fromList(hex.decode(value ? padding + '1' : padding + '0'));
  ...
  static int parseTypeN(String type) {
    return int.parse(RegExp(r'^\D+(\d+)$').firstMatch(type)?.group(1) ?? '1', radix: 10);
  }
  ...
  static BigInt parseNumber(dynamic arg) {
    if (arg is String) {
      if (isHexPrefixed(arg)) {
        return decodeBigInt(hex.decode(stripHexPrefix(arg)));
      } else {
